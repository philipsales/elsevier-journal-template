...
%remarkably however, the storage size differences of in-between groupings had an average 90\% more than the previous. 
%For instance, the difference between first group (i.e. 1,000 records) to the next group (i.e. 10,000 records) is roughly 90\% and the difference for the succeeding group (i.e. 10,000 and 100,000 records) is also about 90\%. 
%This indicates that the data storage storage scaling behavior of all three databases is strongly consistent. 
%However, when the databases were compared to the average size of FHIR Patient resource in its raw JSON format, it was observed that the size changes by substantial degree. Among the three, Cassandra had the best storage rate of 61% decrease in size while the Redis had the most disk storage consumption with increase of 38% from the original file. However, these remarkable storage rate differences are by no means statistically significant when when subjected to inferential computation 
